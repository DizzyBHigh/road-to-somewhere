document.addEventListener('DOMContentLoaded', async () => {
  ...
  const loadReviews = async () => {
    ...
    list.innerHTML = data.length ? data.map(review => {
      ...
      const detailsStyle = 'display:flex;flex-direction:column;align-items:flex-end;gap:3px;text-align:right;';
      return `<article class="review-card"><div class="review-card__top"><div class="review-card__identity"><div class="review-card__details" style="${detailsStyle}"><strong>${escape(name)}</strong>${date ? `<span class="review-card__date">${escape(date)}</span>` : ''}</div><img src="${escape(avatar)}" alt="" class="review-card__avatar" style="${avatarStyle}" onerror="this.style.display='none'"></div><span>${starText(review.rating)}</span></div><p>${escape(review.body)}</p></article>`;
