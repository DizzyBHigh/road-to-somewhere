document.addEventListener('DOMContentLoaded', async () => {
  ...
  const loadReviews = async () => {
    ...
    list.innerHTML = data.length ? data.map(review => {
      ...
      const avatarStyle = 'display:block;width:40px;height:40px;max-width:40px;max-height:40px;min-width:40px;min-height:40px;flex:0 0 40px;border-radius:50%;object-fit:cover;';
