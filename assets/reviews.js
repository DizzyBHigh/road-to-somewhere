document.addEventListener('DOMContentLoaded', async () => {
  ...
  const { data: extension, error: extensionError } = await client.from('extensions')
    .select('id').eq('slug', root.dataset.extensionSlug).single();
  if (extensionError) {
  ...
  const loadReviews = async () => {
    const { data, error } = await client.from('reviews')
      .select('id, user_id, rating, body, created_at, updated_at, status, profiles(display_name, avatar_url)')
      .eq('extension_id', extension.id).eq('status', 'published')
  ...
  if (!user) {
    ...
    form.addEventListener('submit', async event => {
      ...
      if (result.error) {
        message.textContent = `Could not save review: ${result.error.message}`;
      } else {
        message.textContent = 'Review submitted for moderation.';
        state.textContent = 'Your review is awaiting moderation.';
        await loadReviews();
      }
    });
  }
