...
function initDemoPlayers() {
  document.querySelectorAll('[data-demo-player]').forEach(player => {
    ...
    function show(nextIndex) {
      ...
      if (caption) {
        ...
        if (item.title || item.caption) {
          const title = item.title ? document.createElement('strong') : null;
          if (title) { title.textContent = item.title; caption.appendChild(title); }
          if (item.caption) {
    ...
    player.addEventListener('focusout', event => {
      if (!player.contains(event.relatedTarget)) scheduleNext();
    });
    show(0);
  });
}
